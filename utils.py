...
class Utils(QObject):
    left_imageProcessed = pyqtSignal()  # 定义自定义信号
    right_imageProcessed = pyqtSignal()
    LST_Processed = pyqtSignal()

    def detectHarrisCorner(self, img, blocksize=2, ksize=3, k=0.04):
        global left_img
        global left_gray
        global pos
        left_img = cv.imread(img, cv.IMREAD_COLOR)
        left_gray = cv.cvtColor(left_img, cv.COLOR_BGR2GRAY)
        Dx = cv.Sobel(left_gray, cv.CV_32F, 1, 0, ksize=ksize)
        Dy = cv.Sobel(left_gray, cv.CV_32F, 0, 1, ksize=ksize)

        cov = np.zeros([left_gray.shape[0], left_gray.shape[1], 3], dtype=np.float32)

        for i in range(left_gray.shape[0]):
            for j in range(left_gray.shape[1]):
        ...
        for m in range(cov.shape[0]):
            for n in range(cov.shape[1]):
                ...
                b = cov[m, n, 1]
                c = cov[m, n, 2]
                result[m, n] = a * c - b * b - k * (a + c) * (a + c)
        pos = cv.goodFeaturesToTrack(result, 0, 0.05, 10)
        for l in range(len(pos)):
            # point = (int(pos[l][0][0]), int(pos[l][0][1]))
            # cv.circle(left_img, point, 1, [0, 0, 255], thickness=1)
            x = int(pos[l][0][0])
            y = int(pos[l][0][1])
            cv.line(left_img, (x - 5, y), (x + 5, y), (0, 0, 255), 1)
            cv.line(left_img, (x, y - 5), (x, y + 5), (0, 0, 255), 1)

        cv.imwrite("left_match.jpg", left_img)
        self.left_imageProcessed.emit()

    def Harris(self, right_src,kernel_window_size=9, search_window_size=11):
        global left_img
        global left_gray
        global right_img
        global right_gray
        right_img = cv.imread(right_src, cv.IMREAD_COLOR)
            ...
            for k in range(search_window_size):
                for l in range(search_window_size):
                    ...
                    if search_center_x <= 0 or search_center_y <= 0:
                        continue
                    coe = cal_correlation_coefficient(right_gray, sub_left_img, kernel_window_size,
                                                      left_coefficients,
        ...
        for i in range(len(matches)):
            ...
            # cv.circle(right_img, point, 5, [0, 0, 255], thickness=1)
            x = int(matches[i][2])
            y = int(matches[i][3])
            cv.line(right_img, (x - 5, y), (x + 5, y), (0, 0, 255), 1)
            cv.line(right_img, (x, y - 5), (x, y + 5), (0, 0, 255), 1)
        cv.imwrite("right_match.jpg", right_img)
        self.right_imageProcessed.emit()

    def ordinary_least_square(self, window_size = 11):
        global left_gray
        global right_gray
        global matches
        right_img = right_gray
        left_img = left_gray
        match_list = matches
        new_matches = []
        left_points = []
            ...
            while latter_coefficient >= former_coefficient:
                ...
                for m in range(window_size):
                    for n in range(window_size):
                        ...
                        left_dgx = (float(left_img[int(y_1)][int(x_1) + 1]) - float(
                            left_img[int(y_1)][int(x_1) - 1])) / 2.0
                        left_dgy = (float(left_img[int(y_1) + 1][int(x_1)]) - float(
                            left_img[int(y_1) - 1][int(x_1)])) / 2.0
                        factor1 += float(x_1 * left_dgx * left_dgx)
                        factor1_2 += float(left_dgx * left_dgx)
